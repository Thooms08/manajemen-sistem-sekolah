...
<body>
    ...
    <script>
        ...
        // ── Validasi ukuran file — tampilkan feedback di bawah input ─────────────
        ...
        // ── HELPER: Preview gambar — dengan validasi ukuran ──────────────────────
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (tambahModalEl) {
                ...
                tambahModalEl.querySelector('form').addEventListener('submit', function (e) {
                    ...
                    let valid = true;
                    if (logoInput.files[0] && logoInput.files[0].size > MAX_SIZE) {
                        validateFileSize(logoInput, 'feedback-tambah-logo');
                        valid = false;
                    }
                    if (fotoInput.files[0] && fotoInput.files[0].size > MAX_SIZE) {
                        validateFileSize(fotoInput, 'feedback-tambah-foto');
                        valid = false;
                    }
                    if (!valid) {
                        e.preventDefault();
                        Swal.fire({ icon: 'error', title: 'Ukuran file terlalu besar', text: 'Logo dan foto sekolah maksimal 2 MB.', confirmButtonColor: '#198754' });
                    }
                });
                ...
                tambahModalEl.addEventListener('hidden.bs.modal', function () {
                    ...
                    document.getElementById('tambah-preview-foto').innerHTML =
                        `<span class="no-img"><i class="bi bi-image" style="font-size:1.6rem;opacity:.35"></i><br>Unggah foto</span>`;
                    ['feedback-tambah-logo','feedback-tambah-foto'].forEach(id => {
            ...
            document.querySelectorAll('[id^="modalEdit"]').forEach(function (modalEl) {
                ...
                modalEl.addEventListener('shown.bs.modal', function () {
                    ...
                    if (inputLogo && containerLogo) {
                        bindImagePreview(inputLogo, containerLogo, false, `feedback-edit-logo-${id}`);
                    }
                    if (inputFoto && containerFoto) {
                        bindImagePreview(inputFoto, containerFoto, true, `feedback-edit-foto-${id}`);
                    }
                });

                // Block submit edit jika file terlalu besar
                ...
                if (editForm) {
                    editForm.addEventListener('submit', function (e) {
                        ...
                        const inputFoto = modalEl.querySelector('input[name="foto_sekolah"]');
                        let valid = true;
                        if (inputLogo && inputLogo.files[0] && inputLogo.files[0].size > MAX_SIZE) {
                            validateFileSize(inputLogo, `feedback-edit-logo-${id}`);
                            valid = false;
                        }
                        if (inputFoto && inputFoto.files[0] && inputFoto.files[0].size > MAX_SIZE) {
                            validateFileSize(inputFoto, `feedback-edit-foto-${id}`);
                            valid = false;
                        }
                        if (!valid) {
                            e.preventDefault();
            ...
            // ── SweetAlert untuk flash session ───────────────────────────────────
            @if(session('success'))
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
            ...
            @if($errors->any())
                Swal.fire({
                    ...
                    html: `<ul class="text-start small mb-0 ps-3">{!! implode('', array_map(fn($e) => '<li>'.$e.'</li>', $errors->all())) !!}</ul>`,
                    confirmButtonColor: '#198754',
                });
            @endif
        });

        // ── AJAX hapus gambar — pakai SweetAlert ─────────────────────────────────
        function ajaxDeleteImage(id, type) {
            const label = type === 'logo' ? 'Logo Sekolah' : 'Foto Sekolah';
            Swal.fire({
                title: `Hapus ${label}?`,
                text: 'Gambar akan dihapus permanen dan tidak bisa dikembalikan.',
                icon: 'warning',
                ...
                .catch(() => {
                    Swal.fire({ icon: 'error', title: 'Error', text: 'Terjadi kesalahan saat menghapus gambar.', confirmButtonColor: '#198754' });
                });
            });
        }

        // ── Hapus profile — pakai SweetAlert ─────────────────────────────────────
