...
<head>
    ...
    <style>
        ...
        .poster-kop .kop-hari {
            ...
            border-radius: 12px; padding: 8px 20px; text-align: center; flex-shrink: 0;
        }
        ...
        .judul-hari .dot { width: 12px; height: 12px; border-radius: 50%; display: inline-block; }
        ...
        .sesi-card {
            ...
            align-items: center; gap: 16px; border: 1.5px solid #e2e8f0;
        }
    ...
    <div id="content">
        <div style="max-width: 900px; margin: 0 auto;">
            ...
            <div class="hari-nav no-print">
                ...
                @endphp
                <a href="{{ $url }}" class="{{ $isAktif ? 'aktif' : '' }}">
                    {{ $h }}
                    @if($isHariIni)
                        <span style="color:{{ $isAktif ? '#fbbf24' : '#198754' }};font-size:0.68rem;vertical-align:super;font-weight:700;">Hari ini</span>
                    @endif
                </a>
                @endforeach
            </div>
...
<script>
    ...
    async function unduhPNG(e) {
        const btn = e.currentTarget, ori = btn.innerHTML;
        btn.disabled = true;
        ...
        try {
            ...
            if (pH <= a4H) {
                ...
                while (y < iH) {
                    ...
                    pdf.addImage(tmp.toDataURL('image/png'), 'PNG', 0, 0, pW, (tmp.height / iW) * pW);
                    y += slicePx;
                }
            }
