...
class AkunGuruController extends Controller
{
    ...
    {
        // Mengambil data guru beserta akun user-nya secara langsung
        $gurus = Guru::leftJoin('users', 'guru.id_user', '=', 'users.id')
    ...
    public function store(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                // 2. Masukkan ID User yang baru dibuat ke kolom id_user di tabel Guru
                Guru::where('id', $request->id_guru)->update([
                    'id_user' => $user->id
                ]);
            });
    ...
    public function destroy($id_user)
    {
        try {
            DB::transaction(function () use ($id_user) {
                // 1. Putuskan relasi: Kosongkan (set null) id_user di tabel guru
                Guru::where('id_user', $id_user)->update([
                    'id_user' => null
                ]);
                
                // 2. Hapus data akun user
